...
## K. v0.05追加――暗号・標準船・護衛建造の再判定ゲート

### K1. COMSEC

各主要作戦前に、暗号を「強い/弱い」の一値にしない。

- cipher system / code edition
- key distribution latency
- additive/key usage volume
- operator error / repeated text
- capture/compromise
- traffic analysis / DF
- enemy cryptanalytic opportunity
- information expiry

を更新し、敵が**作戦決定に間に合う情報**を得たかを出力する。

特にMO/MIでは、後続70の「1942高等暗号ほぼ解読不能」を入力にしない。

### K2. 戦時標準船

第一次型は開戦時に既に生産パイプラインへ入っているため、研究開始そのものを再抽選しない。
...
- steel/special parts
- repair burden
- invasion/warship priority

で更新する。

第二次型の要求は、第一次型の実運航・故障・休航・損失・荷役を観測してからOPEN→TRIALへ移す。

### K3. ASW建造

護衛幕僚は開戦から限定的に存在。局地司令部、全国司令部、専用護衛艦量産は別スイッチ。

- contacts per convoy-day
- merchant loss / ton-mile loss
- repeated attacks on same convoy
- fleet DD days bound to convoy escort
- sonar/depth-charge/radar supply
- yard/small-engine capacity

を見て、専用護衛艦優先を決める。

### K4. 遣独潜水艦

任務ごとに個艦Availabilityと航路脅威を使う。技術貨物到達を後続資料から固定しない。
