...
def load_combat_support(cp,reg):
    ...
    return sup,fi,load(fi),mx,load(mx),ctx,inp
...
def main():
    ...
    cp_path,cp=resolve_checkpoint(a.checkpoint,idx,graph); cp_id=cp.get('checkpoint_id') or cp.get('id') or (a.checkpoint or idx['current_restart']); graph_node=next((n for n in graph.get('nodes',[]) if n.get('id')==cp_id),None); graph_execution_valid=(graph_node.get('execution_valid') if graph_node is not None else cp.get('execution_valid',True))
    ...
    sup,fi_path,fi,mx_path,mx,ctx_path,input_schema=load_combat_support(cp,reg)
    factor_results,combat_ready,combat_block=resolve_combat_factors(a.profile,cp,selected,gate_by_id,fi,mx,event_input)
    hand={'schema':'EVENT_HANDOUT_SCHEMA_v003','profile':a.profile,'date':(date.isoformat() if date else a.date),'checkpoint':cp_id,'checkpoint_file':str(cp_path.relative_to(ROOT)),'checkpoint_as_of':cp.get('as_of'),'checkpoint_status':cp.get('status'),'checkpoint_execution_valid':graph_execution_valid,'technical_baseline':baseline,'prewar_baseline':cp.get('prewar_baseline'),'current_branch_overrides':ov_path,'active_override_ids':active,'event_profile_revision':prof.get('revision'),'domain_map_revision':dmap.get('revision'),'runtime_contract':contract,'state_sources':cp.get('state_sources',[]),'maturation_deltas':cp.get('maturation_deltas',[]),'domain_resolution':domain_results,'state_requirement_resolution':state_results,'entries':entries,'combat_resolution_support':sup,'combat_factor_index':fi_path,'combat_factor_index_revision':fi.get('revision'),'combat_factor_matrix':mx_path,'combat_factor_matrix_revision':mx.get('revision'),'combat_adjudication_context':ctx_path,'combat_event_input_schema':input_schema,'combat_factor_resolution':factor_results,'combat_resolution_ready':combat_ready,'combat_ready_block_reasons':combat_block,'execution_blocked':bool(block),'block_reasons':block,'integration_rules':['same intermediate state: integrate once','distinct causal stages may compound','check thresholds/reallocation/bottlenecks','carry provisional bands visibly','checkpoint-local state wins over opening baseline fallback','technical cards never substitute for OOB/readiness/personnel state','mandatory combat factors are explicit; omission is not a zero modifier']}
