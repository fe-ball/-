...
reg=load('95_AUDIT/current_version_families_v009.json'); fmap={x.get('family'):x.get('current') for x in reg.get('families',[])}
...
# Entrypoint docs.
readme=(root/'00_README/README_refactor_v012.md').read_text(encoding='utf-8') if exists('00_README/README_refactor_v012.md') else ''
handoff=(root/'00_README/NEXT_SESSION_HANDOFF_REFACTOR_v012.md').read_text(encoding='utf-8') if exists('00_README/NEXT_SESSION_HANDOFF_REFACTOR_v012.md') else ''
...
# v012 session frontier / settlement layer.
front=load('06_RUNTIME/CURRENT_ANALYSIS_FRONTIER_1944SUMMER_v001.json')
if front.get('current_global_analysis_frontier')!='1944-07-14T24:00':err('v012 current analysis frontier mismatch')
if front.get('canonical_restart_checkpoint')!=current:err('v012 frontier canonical restart mismatch')
for rel in front.get('authoritative_local_settlements',[])+front.get('bounded_working_settlements',[]):
    if not exists(rel):err(f'v012 frontier target missing: {rel}')
for x in front.get('forward_local_adjudications_not_global_clock',[]):
    if not exists(x.get('path','')):err(f"v012 forward local adjudication missing: {x.get('path')}")
for rel in ['06_RUNTIME/MARIANAS_SAIPAN_1944-06-11_07-14_SETTLEMENT_v001.md','06_RUNTIME/CBI_AIR_LOGISTICS_TAX_1944-04_06_WORKING_SETTLEMENT_v001.md','06_RUNTIME/CHINA_ICHIGO_ANALOG_1944-05-27_08-06_FORWARD_ADJUDICATION_v001.md','06_RUNTIME/1944SUMMER_JAPAN_US_FORCE_REGENERATION_WORKING_LEDGER_v001.md']:
    if not exists(rel):err(f'v012 settlement missing: {rel}')
if '1944-07-14' not in handoff or '8/6' not in handoff:err('v012 handoff missing frontier/forward-time separation')
if not (root/'99_LEGACY_UNTOUCHED').is_dir():err('legacy subtree missing')
# Final package checks.
if not preflight:
    manifest=load('95_AUDIT/PACKAGE_MANIFEST_v012.json'); checksum_rel=pathlib.Path('95_AUDIT/SHA256SUMS_REFACTOR_v012.txt')
    if manifest:
    ...
    if not exists(checksum_rel):err('v012 checksum missing')
    else:
        listed={}
