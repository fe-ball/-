...
guard=load('06_RUNTIME/BRANCH_B_YEAR_END_GUARD_1944-01-01_v003.json')
reg=load('95_AUDIT/current_version_families_v019.json')
air=load('06_RUNTIME/AIRCRAFT_FIELDING_LEDGER_1943-10-01_v003.json')
kin=load('06_RUNTIME/GALVANIC_KINSEI_ZERO_FIELDING_GATE_1943-11_v002.json')
out=load('06_RUNTIME/GALVANIC_FINAL_OUTCOME_LEDGER_1943-11_12_v002.json')
repair=load('06_RUNTIME/GALVANIC_DAMAGE_REPAIR_REGEN_LEDGER_1943-11_1944Q1_v002.json')
...
if idx.get('current_restart')!='CHECKPOINT_1943-12-31T24_BRANCH_B_v004': err('index restart mismatch')
if cp.get('current_branch_overrides')!='00_CONFIG/current_branch_overrides_v012.json': err('checkpoint override mismatch')
if cp.get('rollback_guard')!='06_RUNTIME/BRANCH_B_YEAR_END_GUARD_1944-01-01_v003.json': err('checkpoint guard mismatch')
...
if nodes.get('CHECKPOINT_1943-12-31T24_BRANCH_B_v004',{}).get('execution_valid') is not True: err('v004 year-end not execution-valid')
if nodes.get('CHECKPOINT_1943-12-31T24_BRANCH_B_v003',{}).get('execution_valid') is not False: err('v003 year-end still execution-valid')
# Final fact checks
...
micro=load('06_RUNTIME/GALVANIC_FIGHTER_MICROCOMBAT_RESULT_1943-11_v001.json')
neg=load('95_AUDIT/GALVANIC_NEGATIVE_DELTA_REVALIDATION_2026-08-29_v001.json')
if micro.get('macro_reinjection',{}).get('centered_route')!='UNCHANGED': err('microcombat route should be unchanged')
if micro.get('macro_reinjection',{}).get('Japanese_working_center')!=67: err('Japanese microcombat macro center mismatch')
if micro.get('macro_reinjection',{}).get('US_working_center')!=44: err('US microcombat macro center mismatch')
if neg.get('qualitative_route_change') is not False: err('negative-delta audit route flag mismatch')
if air.get('post_cutoff_gates',{}).get('GALVANIC_1943_11')!='06_RUNTIME/GALVANIC_KINSEI_ZERO_FIELDING_GATE_1943-11_v002.json': err('aircraft ledger post-cutoff gate missing')
...
if 'OVR-BRANCH-B-GALVANIC-FINAL-CLOSEOUT-003' not in ovs: err('GALVANIC closeout override missing')
...
expected={'CURRENT_BRANCH_STATE_vX.json':'CURRENT_BRANCH_STATE_v012.json','current_branch_overrides_vX.json':'current_branch_overrides_v012.json','CHECKPOINT_INDEX_vX.json':'CHECKPOINT_INDEX_v018.json','CHECKPOINT_GRAPH_vX.json':'CHECKPOINT_GRAPH_v017.json','AIRCRAFT_FIELDING_LEDGER_1943-10-01_vX.json':'AIRCRAFT_FIELDING_LEDGER_1943-10-01_v003.json','BRANCH_B_YEAR_END_GUARD_1944-01-01_vX.json':'BRANCH_B_YEAR_END_GUARD_1944-01-01_v003.json'}
for fam,cur in expected.items():
    if fm.get(fam)!=cur: err(f'registry mismatch {fam}: {fm.get(fam)}')
# Required files
for rel in ['06_RUNTIME/GALVANIC_BRANCH_B_1943-11_SETTLEMENT_v003.md','06_RUNTIME/GALVANIC_FINAL_OUTCOME_LEDGER_1943-11_12_v002.json','06_RUNTIME/GALVANIC_DAMAGE_REPAIR_REGEN_LEDGER_1943-11_1944Q1_v002.json','06_RUNTIME/GALVANIC_AIRCRAFT_AFTER_ACTION_LESSONS_1943-12_v002.md','06_RUNTIME/GALVANIC_KINSEI_ZERO_FIELDING_GATE_1943-11_v002.json','06_RUNTIME/BRANCH_B_1943H2_YEAR_END_SETTLEMENT_v003.md','06_RUNTIME/GALVANIC_FIGHTER_MICROCOMBAT_RESULT_1943-11_v001.json','95_AUDIT/GALVANIC_NEGATIVE_DELTA_REVALIDATION_2026-08-29_v001.json']:
...
if h:
    ...
    if h.get('rollback_guard')!='06_RUNTIME/BRANCH_B_YEAR_END_GUARD_1944-01-01_v003.json': err('generator guard mismatch')
