...
cmd=[sys.executable,str(ROOT/'98_TOOLS/build_event_handout_v009.py'),a.profile]
...
lines=[f"# Combat context: {h['profile']} {h.get('date')}",f"Branch invariant guard: {h.get('branch_invariant_guard')}",f"Checkpoint: {h['checkpoint']} / TECH: {h['technical_baseline']}",f"Aircraft fielding ledger: {h.get('aircraft_fielding_ledger')}",f"Seaplane tactics ledger: {h.get('seaplane_tactics_ledger')}",f"Structural execution blocked: {h['execution_blocked']}",f"Combat resolution ready: {h['combat_resolution_ready']}",f"Weapon/platform layer ready: {h.get('weapon_capability_ready')}","","## Mandatory/conditional capability factors"]
