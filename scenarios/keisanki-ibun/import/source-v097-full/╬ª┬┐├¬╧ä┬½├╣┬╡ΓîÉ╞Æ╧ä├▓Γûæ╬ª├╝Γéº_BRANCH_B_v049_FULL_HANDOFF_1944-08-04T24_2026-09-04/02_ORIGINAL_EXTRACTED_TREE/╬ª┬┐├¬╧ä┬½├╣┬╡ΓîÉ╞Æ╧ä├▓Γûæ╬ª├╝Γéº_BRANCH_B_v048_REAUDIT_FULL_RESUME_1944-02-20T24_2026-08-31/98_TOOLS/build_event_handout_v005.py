...
def opening_ledger(reg):
    name=family(reg,'WARSTART_STATE_LEDGER_vX.json')
    return load('04_WARSTART/'+name)
...
def select(profile,tech):
    ...
    for e in tech.get('entries',[]):
        ...
        domain_relevant=e['domain'] in mand or e['domain'] in also
...
def main():
    reg=latest_family_registry()
    idx=load('05_WARTIME/'+family(reg,'CHECKPOINT_INDEX_vX.json'))
    graph=load('05_WARTIME/'+family(reg,'CHECKPOINT_GRAPH_vX.json'))
    prof=load('06_RUNTIME/'+family(reg,'EVENT_RELEVANCE_PROFILES_vX.json'))
    dmap=load('00_CONFIG/'+family(reg,'DOMAIN_STATE_MAP_vX.json'))
    contract='06_RUNTIME/'+family(reg,'RUNTIME_CONTRACT_vX.json')
    ...
    cp_id=cp.get('checkpoint_id') or cp.get('id') or (a.checkpoint or idx['current_restart'])
    graph_node=next((n for n in graph.get('nodes',[]) if n.get('id')==cp_id),None)
    graph_execution_valid=(graph_node.get('execution_valid') if graph_node is not None else cp.get('execution_valid',True))
    baseline=cp.get('technical_baseline')
    if not baseline: raise SystemExit('selected checkpoint has no technical_baseline')
    tech=load('02_TECH/'+baseline+'.json')
    profile=prof['profiles'][a.profile]
    date=iso_date(a.date or cp.get('as_of'))
    ledger=opening_ledger(reg)
    selected=select(profile,tech)
    cp_declared=set(cp.get('technical_entries',[]))
    entries=[]
    gate_by_id={}
    for e in selected:
        gs,basis=gate_state(e,date); gate_by_id[e['id']]=gs
        rec={'id':e['id'],'domain':e['domain'],'status':e.get('status'),'date_gate_state':gs,'date_gate_basis':basis,'valid_from':e.get('valid_from'),'checkpoint_declared':e['id'] in cp_declared,'causal_stage':e.get('causal_stage',[]),'what_changed':e.get('what_changed'),'sources':e.get('sources',[])}
    ...
    for dom in profile.get('mandatory_domains',[]):
        des=[e for e in selected if e['domain']==dom]
        gates=[gate_by_id[e['id']] for e in des]
        ...
        state_source,bindings=resolve_state_binding(cp,dmap.get('checkpoint_state_paths',{}).get(dom,[]),dmap.get('warstart_map',{}).get(dom,[]),ledger)
        rec={'domain':dom,'resolution':resolution,'entry_ids':[e['id'] for e in des],'state_source':state_source,'state_bindings':bindings}
        domain_results.append(rec)
    ...
    for req in profile.get('state_requirements',[]):
        ...
        if not spec:
            state_results.append({'requirement':req,'resolution':'UNKNOWN','bindings':[]}); block.append(f'state requirement definition missing: {req}'); continue
        source,bindings=resolve_state_binding(cp,spec.get('checkpoint_paths',[]),spec.get('warstart_domains',[]),ledger)
        state_results.append({'requirement':req,'resolution':source,'bindings':bindings})
        if source=='UNKNOWN':block.append(f'mandatory state requirement UNKNOWN: {req}')
    ov_path=cp.get('current_branch_overrides')
    active=[]
    if ov_path and (ROOT/ov_path).exists():
        od=load(ov_path)
        active=[o.get('override_id') or o.get('rule_id') for o in od.get('overrides',[]) if o.get('status') in ('ACTIVE_CURRENT_BRANCH','active')]
    hand={
      'schema':'EVENT_HANDOUT_SCHEMA_v002','profile':a.profile,'date':(date.isoformat() if date else a.date),'checkpoint':cp_id,'checkpoint_file':str(cp_path.relative_to(ROOT)),
      'checkpoint_as_of':cp.get('as_of'),'checkpoint_status':cp.get('status'),'checkpoint_execution_valid':graph_execution_valid,
      'technical_baseline':baseline,'prewar_baseline':cp.get('prewar_baseline'),'current_branch_overrides':ov_path,'active_override_ids':active,
      'event_profile_revision':prof.get('revision'),'domain_map_revision':dmap.get('revision'),'runtime_contract':contract,
      'state_sources':cp.get('state_sources',[]),'maturation_deltas':cp.get('maturation_deltas',[]),
      'domain_resolution':domain_results,'state_requirement_resolution':state_results,'entries':entries,
      'execution_blocked':bool(block),'block_reasons':block,
      'integration_rules':['same intermediate state: integrate once','distinct causal stages may compound','check thresholds/reallocation/bottlenecks','carry provisional bands visibly','checkpoint-local state wins over opening baseline fallback','technical cards never substitute for OOB/readiness/personnel state']}
