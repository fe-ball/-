...
reg=load('95_AUDIT/current_version_families_v016.json'); fm={x.get('family'):x.get('current') for x in reg.get('families',[])}
...
state=load('06_RUNTIME/CURRENT_BRANCH_STATE_v009.json'); idx=load('05_WARTIME/CHECKPOINT_INDEX_v015.json'); graph=load('05_WARTIME/CHECKPOINT_GRAPH_v014.json'); cp=load('05_WARTIME/CHECKPOINT_1943-12-31T24_BRANCH_B_v001.json'); guard=load('06_RUNTIME/BRANCH_B_YEAR_END_GUARD_1944-01-01_v001.json'); ov=load('00_CONFIG/current_branch_overrides_v009.json'); tech=load('02_TECH/TECH_INDEX_v008.json')
...
if idx.get('current_restart')!='CHECKPOINT_1943-12-31T24_BRANCH_B_v001': err('checkpoint index current restart mismatch')
if idx.get('graph')!='CHECKPOINT_GRAPH_v014.json': err('checkpoint index graph mismatch')
if state.get('active_restart',{}).get('checkpoint')!='CHECKPOINT_1943-12-31T24_BRANCH_B_v001': err('state restart mismatch')
...
if cp.get('current_branch_overrides')!='00_CONFIG/current_branch_overrides_v009.json': err('override binding missing')
...
for oid in ['OVR-BRANCH-B-RESTART-1944-01-01-001','OVR-BRANCH-B-1943H2-REALIZED-001','OVR-BRANCH-B-RN-AUDIT-001','OVR-BRANCH-B-1944-YEAREND-GUARD-001','OVR-BRANCH-B-JAPAN-NAVAL-WALLET-FIRST-001']:
    if oid not in active: err('active Branch B override missing '+oid)
nodes={n.get('id'):n for n in graph.get('nodes',[])}
if not nodes.get('CHECKPOINT_1943-12-31T24_BRANCH_B_v001',{}).get('execution_valid'): err('year-end current node not execution-valid')
...
expected={'CURRENT_BRANCH_STATE_vX.json':'CURRENT_BRANCH_STATE_v009.json','current_branch_overrides_vX.json':'current_branch_overrides_v009.json','CHECKPOINT_INDEX_vX.json':'CHECKPOINT_INDEX_v015.json','CHECKPOINT_GRAPH_vX.json':'CHECKPOINT_GRAPH_v014.json','build_event_handout_vX.py':'build_event_handout_v012.py','build_combat_context_vX.py':'build_combat_context_v007.py'}
...
if h:
    if h.get('checkpoint')!='CHECKPOINT_1943-12-31T24_BRANCH_B_v001': err('generator default checkpoint mismatch')
    ...
    if h.get('current_branch_overrides')!='00_CONFIG/current_branch_overrides_v009.json': err('generator override missing')
