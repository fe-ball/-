...
cmd=[sys.executable,str(ROOT/'98_TOOLS/build_event_handout_v007.py'),a.profile]
...
lines=[f"# Combat context: {h['profile']} {h.get('date')}",f"Checkpoint: {h['checkpoint']} / TECH: {h['technical_baseline']}",f"Structural execution blocked: {h['execution_blocked']}",f"Combat resolution ready: {h['combat_resolution_ready']}",f"Weapon/platform layer ready: {h.get('weapon_capability_ready')}","","## Mandatory/conditional capability factors"]
