...
front=load('00_README/CURRENT_DISCUSSION_FRONTIER_v011.json'); reg=load('95_AUDIT/current_version_families_v029.json'); tech=load('02_TECH/TECH_INDEX_v009.json')
...
m1=load('06_RUNTIME/MARSHALL_AIR_OOB_1944-01-28_WORKING_v001.json'); m2=load('06_RUNTIME/MARSHALL_AIR_OOB_1944-01-28_WORKING_v003.json')
...
if front.get('authority_frontier',{}).get('changed_by_v042') is not False:err('v042 must not promote authority')
if front.get('discussion_frontier',{}).get('discussion_clock_center')!='1944-02-03T15:30:00+12:00':err('discussion clock mismatch')
...
if m2.get('role_total_center',{}).get('fighter_or_local_cover')!=72:err('Marshall v003 fighter/local-cover center not 72')
# fighter slots center = 72, no addition
fighters=m2.get('role_total_center',{}).get('fighter_or_local_cover')
...
exp={
 'README_refactor_vX.md':'README_refactor_v042.md','NEXT_SESSION_HANDOFF_REFACTOR_vX.md':'NEXT_SESSION_HANDOFF_REFACTOR_v042.md','PACKAGE_MANIFEST_vX.json':'PACKAGE_MANIFEST_v042.json','VALIDATION_RESULT_vX.json':'VALIDATION_RESULT_v042.json','validate_refactor_vX.py':'validate_refactor_v025.py','CURRENT_DISCUSSION_FRONTIER_vX.json':'CURRENT_DISCUSSION_FRONTIER_v011.json','current_version_families_vX.json':'current_version_families_v029.json',
 'MARSHALL_AIR_OOB_1944-01-28_WORKING_vX.json':'MARSHALL_AIR_OOB_1944-01-28_WORKING_v003.json','TECHNOLOGY_LINEAGE_SYNTHESIS_1944-01-28_WORKING_vX.json':'TECHNOLOGY_LINEAGE_SYNTHESIS_1944-01-28_WORKING_v003.json','RAIDEN_INTERCEPTOR_SYSTEM_1943Q4_1944Q1_WORKING_vX.json':'RAIDEN_INTERCEPTOR_SYSTEM_1943Q4_1944Q1_WORKING_v001.json','AIR_DEFENSE_EXCHANGE_TRANSITION_POST_GALVANIC_1943-11_1944-01_WORKING_vX.json':'AIR_DEFENSE_EXCHANGE_TRANSITION_POST_GALVANIC_1943-11_1944-01_WORKING_v001.json','1944SUMMER_JAPAN_US_FORCE_REGENERATION_WORKING_LEDGER_vX.md':'1944SUMMER_JAPAN_US_FORCE_REGENERATION_WORKING_LEDGER_v002.md','RAIDEN_HEAVY_BOMBER_INTERCEPT_REAUDIT_2026-08-29_vX.md':'RAIDEN_HEAVY_BOMBER_INTERCEPT_REAUDIT_2026-08-29_v001.md','JAPAN_FIRST_LINE_CARRIER_HOLDING_FORCE_1944-01-28_02-02_WORKING_vX.json':'JAPAN_FIRST_LINE_CARRIER_HOLDING_FORCE_1944-01-28_02-02_WORKING_v003.json','FLINTLOCK_SECOND_SUPPRESSION_CYCLE_1944-02-02_WORKING_vX.json':'FLINTLOCK_SECOND_SUPPRESSION_CYCLE_1944-02-02_WORKING_v002.json'}
...
# v042 carrier-exchange checks
sec=load('06_RUNTIME/FLINTLOCK_SECOND_SUPPRESSION_CYCLE_1944-02-02_WORKING_v002.json')
if sec.get('US_second_cycle',{}).get('center')!=194:err('v041 sortie-center repair missing')
if sec.get('Japanese_defense',{}).get('center')!=34:err('v041 fighter-center repair missing')
if sec.get('losses_second_cycle_only',{}).get('US_total_irrecoverable_center')!=11:err('v041 loss-center repair missing')
...
post=load('06_RUNTIME/JAPAN_FIRST_LINE_CARRIER_BATTLE_STATE_1944-02-03T1530_WORKING_v001.json')
if post.get('ships',{}).get('Hiryu')!='MOBILE_FLIGHT_OPS_SUSPENDED_PENDING_REPAIR':err('Hiryu postbattle state changed')
local=load('06_RUNTIME/MARSHALL_AIR_OOB_1944-02-03T1530_WORKING_v001.json')
if local.get('theatre_1530',{}).get('combat_serviceable_center')!=47:err('Marshall Feb3 center changed')
